...
set lastsuccessfuldataextrndate =  NULL
where id IN (57);
...
select		cast(item_cd as varchar) as skuID,
			...
			replace(translate(dih.cons_item_desc, ''~#$%\}<>{?*!"'',''!!!!!!!!!!!!!''),''!'','''') as product_desc,
...
where id IN (57);
  ...
  SELECT id, feedname, lastsuccessfuldataextrndate, lastfaileddataextrndate, lastextrnstatus
  FROM [metadata].[ConfigFileExtractor]
  where id = '57';
